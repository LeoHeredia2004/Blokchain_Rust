...
fn generate_small_prime() -> u64 {
    ...
    loop {
        let n = rng.gen_range(10000..50000);
        if is_prime(n) {
            return n;
        }
    }
...
pub fn encrypt_string(mensagem: &[u64], expoente: u64, modulo: u64) -> Vec<u64> {
    mensagem.iter().map(|&m| {
        // 'm' já é o u64 que precisamos.
        // A linha 'let m = c as u8 as u64;' foi removida.
        mod_exp(m, expoente, modulo)
    }).collect()
}
